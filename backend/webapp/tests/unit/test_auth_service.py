...
def test_login_should_return_authenticated_user_dto():
    ...
    repo.get_user_by_email = Mock(
        return_value=RegisteredUserDTO(
            email=EMAIL, password_hash=password_hash, role=Role("user"), is_active=True
        )
    )

    result = service.login(login_data=UserLoginInputDTO(email=EMAIL, password=PASSWORD))
...
def test_login_should_return_none_for_invalid_credentials():
    ...
    repo.get_user_by_email = Mock(
        return_value=RegisteredUserDTO(
            email=EMAIL, password_hash=password_hash, role=Role("user"), is_active=False
        )
    )
    result = service.login(login_data=UserLoginInputDTO(email=EMAIL, password=PASSWORD))
...
def test_register_should_return_authenticated_user_dto():
    ...
    repo.create_user = Mock(
        return_value=RegisteredUserDTO(
            email=EMAIL, password_hash="irrelevant", role=Role("user"), is_active=True
        )
    )
